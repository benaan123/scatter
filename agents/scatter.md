...
### Step 4: Daily plan check

For the active project (the one the user is most likely working on today — infer from station.toml or ask):

```bash
$BIN/vault daily-plan <project>
```

- **If no plan existed** (template was just created): Tell the user a daily plan template was created in their Obsidian vault. Ask what today's priorities are. When they respond, write the priorities into the plan file using `sed` or by rewriting the file.
- **If a plan exists**: Show a brief summary of the priorities and note which items are already checked off. This orients both you and the user for the session.
...
When spawning multiple teams from a single user message, fire ALL spawn-lead commands as parallel Bash tool calls in a SINGLE response. Example:

- Bash call 1: `$BIN/spawn-lead --name auth-team ...`
- Bash call 2: `$BIN/spawn-lead --name api-team ...`
- Bash call 3: `$BIN/spawn-lead --name docs-team ...`

All three run concurrently. spawn-lead uses file locking to handle tmux layout safely. Do NOT wait for one spawn to complete before starting the next.
...
5. Run: `$BIN/vault ping <project>` — remind the user what priorities remain and suggest what to work on next
6. Offer the user a review before sweeping
...
When the user signals they're done for the day ("done for today", "wrapping up", "that's it for now", "EOD", etc.):

1. Summarize what was accomplished across all teams that ran this session
2. Run the wrap-up command with your summary:
   ```bash
   $BIN/vault wrap-up <project> "Summary of what was accomplished today"
   ```
3. Run `$BIN/vault update-status <project>` to refresh the project status
4. Offer to sweep any remaining completed teams

This ensures the daily plan file in Obsidian has a complete record of the session — priorities set at the start, wrap-up at the end — which carries forward to the next day's plan.
